...
def clean_subtitle_text(text):
    """
    자막 텍스트에서 설명문(SDH)이나 잡음을 제거합니다.
    - (괄호), [대괄호] 안의 내용 제거 (화자 표시, 상황 설명 등)
    - (?) 같은 불확실 표시 제거
    - 특수문자 및 공백 정리
    """
    if not text:
    ...
    # 1. 괄호 내용 제거: (글자), [글자]
    # 비탐욕적(non-greedy) 매칭으로 괄호 쌍을 찾음
    text = re.sub(r'\([^)]*\)', '', text)
    text = re.sub(r'\[[^\]]*\]', '', text)
    
    # 2. 열린 괄호만 있고 닫히지 않은 경우 처리 (가끔 발생)
    text = text.replace('(', '').replace(')', '')
    text = text.replace('[', '').replace(']', '')

    # 3. 물음표(?) 제거 (자막에서 불확실함 표시로 쓰임)
    text = text.replace('?', '?') # 단순화
...
def calculate_difficulty(sentence, duration):
    ...
    # 구어체 종결어미 확인 (필수)
    # 요, 다, 까, 죠, 야, 니, 지, 돼, 해, 봐
    spoken_endings = ['요', '다', '까', '죠', '야', '니', '지', '돼', '해', '봐', '서', '게', '나', '네', '데']
    if not any(sentence.strip().endswith(ending) for ending in spoken_endings):
        return 999  # 명사형/자막형 문장은 난이도 폭탄 -> 탈락시킴
